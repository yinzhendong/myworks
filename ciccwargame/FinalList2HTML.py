...
def get_qualified_list():
    ...
    query_qualified_person = (
        "select (@rowNum:=@rowNum+1) num, area, name, phone_conv "
        "from person_final, (SELECT @rowNum:=0) t "
        "where state='' or state is null order by area"
    )

    query_qualified_team = (
        "select (@rowNum:=@rowNum+1) num, area, team_name, leader_name, "
        "leader_phone_conv, member_name, member_phone_conv "
    ...
    # 全国晋级赛名单
    with open('list.html', 'w',encoding='utf-8') as html:
        html.write('<!DOCtype HTML>'
                   ...
                   '<body>')
        html.write('<br><hr />')
        html.write('<h1 align=center>全国总决赛晋级赛--个人赛名单</h1>')
        ...
        # 全国晋级赛编队赛名单
        html.write('<h1 align=center>全国总决赛晋级赛--编队赛名单</h1>')
        html.write('<table border=1 align=center>')
        html.write('<tr align=center>'
                   '<th>序号</th>'
                   '<th>所属赛区</th>'
