...
def create_country_top(date, count):
    ...
    query_country_person_rank = (
        ...
        "max_score as 最高成绩 "
        "FROM person, (select @i:=0)t WHERE max_score IS NOT NULL "
        "ORDER BY 最高成绩 DESC "
        "LIMIT {}".format(count)
    )
    ...
    query_country_team_rank = (
        ...
        "ORDER BY 总成绩 DESC LIMIT {}".format(count)
    )
    ...
    # 个人赛成绩TOP10
    with open('top.html', 'w',encoding='utf-8') as html:
        html.write('<!DOCtype HTML>'
                   ...
                   '<body>')
        html.write('<br><hr />')
        html.write('<h1 align=center>个人赛--全国排名' + str(count) +'强</h1>')
        ...
        # 编队成绩TOP10
        html.write('<h1 align=center>编队赛--全国排名' + str(count) + '强</h1>')
        html.write('<h2 align=center>成绩统计时间：' + date + '</h2>')
        html.write('<table border=1 align=center>')
        html.write('<tr align=center>'
                   '<th>全国排名</th>'
